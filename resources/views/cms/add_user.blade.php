...
@section('content')
  ...
  <div class="row">
    <div class="col-6">
      <form action="{{ url('cms/users') }}" method="POST" novalidate autocomplete="off">
        @csrf

        <input type="hidden" name="eval_pw" value="yes">
            ...
            <div class="form-group">
              ...
              <select name="role" id="role" class="form-control select-fix">
                @foreach ($roles as $item)
                  <option @if (old('role') == $item['id']) selected @endif value="{{ $item['id'] }}">{{ $item['role'] }}</option>
                @endforeach
              </select>
              <small class="text-muted">Choose role of user</small><br>
            ...
            <div class="form-group">
              ...
              <select name="status" id="status" class="form-control select-fix">
                @foreach ($statuses as $item)
                  <option @if (old('status') == $item['id']) selected @endif value="{{ $item['id'] }}">{{ $item['status'] }}</option>
                @endforeach
              </select>
              <small class="text-muted">Choose account status</small><br>
        ...
        <div class="form-group">
          ...
          <input type="text" name="name" id="name" class="form-control field-input-cms mb-1" value="{{ old('name') }}">
        ...
        <div class="form-group">
          ...
          <input type="email" name="email" id="email" class="form-control field-input-cms mb-1" value="{{ old('email') }}">
        ...
        <input type="submit" value="Save User" name="submit" class="btn btn-lg btn-primary mt-3">
